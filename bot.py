...
class TicketCloseConfirmView(discord.ui.View):
    ...
    async def confirm_close(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if interaction.user.id != int(data["user_id"]) and not is_ticket_staff(interaction) and not is_allowed(interaction):
            await interaction.followup.send("チケット作成者またはスタッフのみ閉じることができます。", ephemeral=True)
            return
        ...
        try:
            ...
            await interaction.followup.send(f"クローズ処理中にエラーが発生しました: {e}", ephemeral=True)
...
class TicketControlView(discord.ui.View):
    ...
    async def close_ticket(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if interaction.user.id != int(data["user_id"]) and not is_ticket_staff(interaction) and not is_allowed(interaction):
            ...
            return
        await interaction.response.send_message("本当にチケットを閉じますか？", view=TicketCloseConfirmView(), ephemeral=True)
...
class EmbedEditLoadModal(discord.ui.Modal, title="既存Embedを読み込む"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        try:
            ...
            await interaction.followup.send("無効なチャンネルIDです。数字で入力してください。", ephemeral=True)
            return
...
async def on_ready():
    ...
    stale_ticket_keys = [
        ch_id for ch_id, tdata in list(ticket_data.items())
        if tdata.get("status") == "closed"
        and not any(
            g.get_channel(int(ch_id)) for g in client.guilds
            if str(g.id) == tdata.get("guild_id")
        )
    ]
    for key in stale_ticket_keys:
        ticket_data.pop(key, None)
    if stale_ticket_keys:
        save_ticket_data(ticket_data)
