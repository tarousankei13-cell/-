...
プレイヤーがブラウザを閉じている間のRollは、スイッチなしで自動的にたまり、次に開いた
ときにまとめて処理されます。全員に効くので、配りすぎだと感じたら管理パネルの
Settings → offline で調整してください。

| 設定 | 既定 | 意味 |
|---|---|---|
| `features.offline_roll_enabled` | ON | オフライン加算そのものの有効/無効 |
| `offline.efficiency` | 0.6 | 離席時間のうちRollに数える割合（装備のオフライン効率が加算） |
| `offline.max_hours` | 8 | 1回に数える離席時間の上限（オフライン処理装置I/IIで +4h / +8h） |
| `offline.min_gap_seconds` | 45 | これより短い間隔は「離席」と見なさない |
| `offline.max_rolls` | 40000 | 1回の処理で行うRoll数の上限 |
